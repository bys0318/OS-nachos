...
public class Alarm {
    ...
    public void timerInterrupt() {
        ...
        while (!waitQueue.isEmpty() && waitQueue.peek().time <= Machine.timer().getTime()){
            AlarmThread alarmThread = waitQueue.poll();
            alarmThread.thread.ready();
        }
    ...
    public void waitUntil(long x) {
        ...
        AlarmThread alarmThread = new AlarmThread(thread, wakeTime);
        boolean inStatus = Machine.interrupt().disable();
    ...
    private class AlarmThread implements Comparable<AlarmThread>{
        ...
        public int compareTo(AlarmThread a){
            return this.time < a.time ? -1 : 1;
        }
        public KThread thread;
        public long time;
    }
    
    private PriorityQueue<AlarmThread> waitQueue = new PriorityQueue<AlarmThread>();
}
